...
class CbwooplainsalesstatAdmin {
    ...
    private function __construct() {
		...
		add_filter('cbwooplainsalesstat_nav_button_monthly', array($this, 'cbwooplainsalesstat_nav_button_monthly'), 10, 3);
...
//    }
    ...
    public function cbwooplainsalesstat_add_plugin_admin_menu() {
        ...
        $this->plugin_screen_hook_suffix = add_submenu_page(
                'woocommerce', __('CBX Sales Report', $this->plugin_slug), __('CBX Sales Report', $this->plugin_slug), 'manage_options', $this->plugin_slug, array($this, 'cbwooplainsalesstat_display_plain_stat'), plugins_url('assets/css/sale.png', __FILE__)
        );

        add_options_page(
                __('CBX Woo Sales Report Setting', $this->plugin_slug), __('CBX Woo Sales Report Setting', $this->plugin_slug), 'manage_options', 'cbwooreportsettings', array($this, 'display_plugin_admin_page')
        );
    }
    ...
    public static function cbwooplainsalesstat_get_sale($cbday = '', $cbmonth = '', $cbyear = '') {
        ...
        return array('orders' => $cbxstat_orders,
    ...
    public static function cbwooplainsalesstat_display_nav($active = 3) {
        ...
        $cbxpainstat_values = is_array($cbxpainstat) ? array_values($cbxpainstat) : array('on', 'on', 'on', 'on', 'on', 'on', 'on');
    ...
    public function cbwooplainsalesstat_display_plain_stat() {
        ...
        //for chart settings
        $chart_settings        = apply_filters('cbxwoo_display_plain_stat_chart_settings', array($plugin_slug . 'chartmonthlystat' => 1, $plugin_slug . 'chartlistmonthlystat' => array('line' => 'line')), $plugin_slug);
        $chart_settings_optval = get_option($plugin_slug . 'chart_settings', array($plugin_slug . 'chartmonthlystat' => 1, $plugin_slug . 'chartlistmonthlystat' => array('line' => 'line')));
	...
	public function cbwooplainsalesstat_nav_button_monthly($extra_nav_button, $year, $month){
		...
		<a class="button" href="' . admin_url('admin.php?page=cbwooplainsalesstat&export=pdf&type='.$type.'&cbstatmonth='.$cbstatmonth.'&cbstatyear='.$cbstatyear) . '">' . __('Export as Pdf', 'cbwooplainsalesstat') . '</a>
	';
